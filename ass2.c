...
void insertElement(int *A, int size) {
    ...
    scanf("%d", &element);
    printf("Enter the position (0 to %d) where you want to insert the element: ", size-1);
    scanf("%d", &position);
...
void linearSearch(int *A, int size, int target) {
    int found = 0;//linear searching
    printf("Enter the element you have to find by linear searching: ");
    scanf("%d", &target);
    ...
    for (int i = 0; i < size; i++) {
        ...
        if (*(A + i) == target) {
            ...
            printf("The target element is present at index number: %d\n", i);
            found = 1;
            break;
        }
    }
